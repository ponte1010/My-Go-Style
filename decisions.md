- [Goスタイル決定事項](#goスタイル決定事項)
  ...
  - [テスト構造](#テスト構造)
    ...
    - [テストヘルパー](#テストヘルパー)
    - [テストパッケージ](#テストパッケージ)
      - [同一パッケージ内のテスト](#同一パッケージ内のテスト)
      - [別パッケージのテスト](#別パッケージのテスト)
    - [`testing`パッケージの使用](#testingパッケージの使用)
  - [比決定事項](#比決定事項)
...
注意: このドキュメントは、私の[Goスタイル](https://google.github.io/styleguide/go/index)の概要を説明する一連のドキュメントの一部です。このドキュメントは規範的なものですが、標準的なものではなく、コアスタイルガイドに従属するものです。詳しくは、概要をご覧ください。
...
`strings`パッケージのような標準ライブラリ関数には、インバンドエラー値を返すものがあります。これは文字列操作のコードを非常に単純化しますが、その代償としてプログラマはより多くの注意を払わなければなりません。一般的に、私のコードベースにおけるGoのコードは、 エラーに対して追加の値を返すべきです。
...
Stubby RPCクライアントを使用するパッケージをテストするには、本物のクライアント接続を使用します。本物のサーバがテストに使えない場合、rpctestパッケージを使ってローカルの[テストダブル](https://abseil.io/resources/swe-book/html/ch13.html#techniques_for_using_test_doubles)に本物のクライアント接続を取得するようにしています（近日公開予定）。
...
私のコードベースにおけるGoプログラムは、[標準の`flag`パッケージ](https://golang.org/pkg/flag/)の内部バージョン を使用しています。これは似たようなインターフェイスですが、内部システムとの相互運用性が高いです。Goバイナリのフラグ名は、アンダースコアで単語を区切るのが好ましいですが、 フラグの値を保持する変数は標準の Go名スタイル（[ミックスキャップ](guide.md#ミックスキャップ）に従わなければなりません。具体的には、フラグ名はスネークケースで、変数名はキャメルケースで同等の名前にします。
...
私のコードベースのGoプログラムは、[標準の`log`パッケージ](https://pkg.go.dev/log)の変種を使用しています。類似していますがより強力なインターフェイスで、Googleの内部システムともうまく連動しています。このライブラリのオープンソース版は[package glog](https://pkg.go.dev/github.com/golang/glog)として提供されており、 オープンソースのGoogleプロジェクトではそちらを使用することができますが、 このガイドでは全体を通して`log`と表記します。
...
私のコードベースの中で、親コンテキストがキャンセルされた後に実行できるバックグラウンド操作を生成しなければならないコードは、デタッチメントのために内部パッケージを使用することができます。オープンソースの代替案については、[issue #40221](https://github.com/golang/go/issues/40221)を参照してください。
...
所有していないパッケージの出力の安定性に依存するような結果の比較は避けてください。その代わり、テストは安定していて依存関係の変化に強い、 意味的に関連性のある情報で比較すべきです。フォーマットされた文字列やシリアル化されたバイトを返す機能については、一般にその出力が安定していると仮定するのは安全ではありません。

たとえば、[`json.Marshal`](https://golang.org/pkg/encoding/json/#Marshal)は出力する特定のバイトを変更することができます（過去に変更したこともあります）。JSON 文字列に対して文字列の等式処理を行うテストは、`json`パッケージがバイトをシリアライズする方法を変更した場合に壊れてしまう可能性があります。その代わりに、より堅牢なテストはJSON文字列の内容を解析し、それが期待されるデータ構造と意味的に等価であることを確認します。

### 継続

テストは、失敗したチェックをすべて一度にプリントするために、失敗した後でもできるだけ長く続ける必要があります。こうすれば、失敗したテストを修正している開発者が、 バグを修正するたびにテストを再実行し、 次のバグを見つける必要がなくなります。

ミスマッチの報告には、`t.Fatal`よりも`t.Error`を優先してください。ある関数の出力について複数の異なる属性を比較する場合、それらの比較のそれぞれについて`t.Error`を使用します。

`t.Fatal`の呼び出しは、主に予期せぬエラー状態を報告する際に有用で、その後の比較の失敗が意味をなさない場合です。

テーブル駆動テストでは、サブテストを使用し、`t.Error`や`continue`よりも`t.Fatal`を使用することを検討しましょう。[GoTip #25: サブテスト: テストを無駄のないものにする](テストを無駄のないものにする。)も参照ください。

**ベストプラクティス**: `t.Fatal`をいつ使うべきかということについては、[ベストプラクティス](#TBD)を参照してください。

### 等値比較と差分

`==`演算子は、[言語で定義された比較](http://golang.org/ref/spec#Comparison_operators)を使って等質性を評価します。スカラー値（数値、ブール値など）はその値に基づいて比較されますが、この方法で比較できるのは一部の構造体とインターフェースだけです。ポインターは、指す値が等しいかどうかではなく、同じ変数を指しているかどうかで比較されます。

[cmp](https://pkg.go.dev/github.com/google/go-cmp/cmp)パッケージでは、スライスなど`==`では適切に処理されない複雑なデータ構造を比較できます。等値比較には[cmp.Equal](https://pkg.go.dev/github.com/google/go-cmp/cmp#Equal)を使用し、オブジェクト間の差分を取得するには[cmp.Diff](https://pkg.go.dev/github.com/google/go-cmp/cmp/cmp#Diff)を使用します。
...
汎用比較ライブラリである`cmp`は、特定の型を比較する方法を知らない場合があります。たとえば、[protocmp.Transform](https://pkg.go.dev/google.golang.org/protobuf/testing/protocmp#Transform)オプションが渡された場合のみ、プロトコルバッファーメッセージを比較することができます。
...
`cmp`パッケージはGo標準ライブラリーの一部ではありませんが、Goチームによってメンテナンスされており、長期にわたって安定した比較結果が得られるはずです。ユーザー設定可能で、ほとんどの比較ニーズに対応できます。

既存のコードでは、以下の古いライブラリーを使用することが可能で、一貫性を保つためにそれらを使い続けることができます。

- [pretty](https://pkg.go.dev/github.com/kylelemons/godebug/pretty)は視覚的に美しい差分レポートを作成します。しかし、これは意図的に同じ視覚表現を持つ値を等しいと見なします。特に、`pretty`はnilスライスと空のスライスの違いを捉えず、同一のフィールドを持つ異なるインターフェイスの実装にも影響を与えず、構造体の値との比較の基礎として入れ子のマップを使用することが可能です。また、diff を生成する前に値全体を文字列にシリアライズするため、大きな値の比較には適していません。デフォルトでは、エクスポートされていないフィールドを比較するため、依存関係にある実装の詳細が変更された場合に影響を受けやすくなります。このため、protobufメッセージに対して`pretty`を使用することは適切ではありません。

新しいコードでは`cmp`を使用することを優先し、古いコードでも`cmp`を使用するように更新することを検討する価値があります。

古いコードでは、複雑な構造の比較に標準ライブラリ`reflect.DeepEqual`関数が使用されている場合があります。`reflect.DeepEqual`は、エクスポートされていないフィールドやその他の実装の詳細の変更に影響を受けやすいため、等価性のチェックに使用するべきではありません。`reflect.DeepEqual`を使用しているコードは、上記のライブラリのいずれかに更新する必要があります。

古いコードでは、複雑な構造の比較に標準ライブラリ`reflect.DeepEqual`関数が使用されている場合があります。`reflect.DeepEqual`は、エクスポートされていないフィールドやその他の実装の詳細の変更に影響を受けやすいため、等価性のチェックに使用するべきではありません。`reflect.DeepEqual`を使用しているコードは、上記のライブラリのいずれかに更新する必要があります。

**注意**: `cmp`パッケージは、プロダクション環境での使用ではなくテスト用に設計されています。そのため、比較の実行が正しくないと疑われる場合にパニックが発生し、テストの脆弱性を改善する方法をユーザーに指示する場合があります。`cmp`はパニックを起こす性質があるため、プロダクションで使用するコードには適していません。

### 詳細度

従来の失敗メッセージは、ほとんどの Go テストに適しており、`YourFunc(%v) = %v, want %v`となります。 しかし、もっと詳細が必要な場合もありますし、もっと少ない場合もあります。

- 複雑なやりとりをするテストは、そのやりとりも記述しなければなりません。たとえば、同じ YourFunc が何度もコールされた場合、どのコールがテストに失敗したのかを特定します。もしシステムの状態を知ることが重要なら、失敗時の出力 （あるいは少なくともログ）にそれを含めてください。
- データが複雑な構造体で定型文が多い場合は、メッセージに重要な部分のみを記述しても構いませんが、データを過度に隠さないようにしてください。
- セットアップの失敗については、同じレベルの詳細さは必要ありません。`t.Fatalf("Setup: Failed to set up test database: %s", err)`は、通常、問題を解決するのに十分役に立ちます。

**ヒント**: 開発時には、失敗モードのトリガーを作っておきましょう。失敗メッセージがどのようなものか、メンテナが効果的に対処できるかを検討しましょう。

テストの入出力を明確に再現するためのテクニックがいくつかあります。

- 文字列データをプリントする場合、値が重要であることを強調し、間違った値をより簡単に発見するために、[`%q`がよく役に立ちます](#qの使用)。
- （小さな）構造体をプリントする場合、`%v`よりも`%+v`の方が有効な場合があります。
- 大きな値の検証に失敗した場合、[差分をプリント](#差分のプリント)することでその失敗を容易に理解することができます。

### 差分をプリント

関数が大きな出力を返す場合、テストが失敗したときに失敗メッセージを読む人がその差分を見つけるのは難しいでしょう。返される値と期待される値の両方を表示するのではなく、差分を取るようにしましょう。

このような値の差分を計算するには、特に新しいテストや新しいコードでは`cmp.Diff`を使用するのがよいでしょう。各関数の長所と短所に関するガイダンスについては、「等値性の種類」を参照してください。
...
[`diff`](https://pkg.go.dev/github.com/kylelemons/godebug/diff)パッケージを使用すると、複数行の文字列や文字列のリストを比較することができます。これを他の種類の差分のための構成要素として使用することができます。

失敗メッセージに diff の方向を説明するテキストを追加してください。

- `diff (-want +got)`のようにすると、`cmp`、`pretty`、`diff`パッケージを使用している場合 （関数に`(want, got)`を渡した場合）は、フォーマット文字列に追加した`-`と`+`が実際に差分の行の先頭に現れる`-`と`+`と一致するのでよいでしょう。関数に`(got, want)`を渡した場合、正しいキーは`(-got +want)`となります。
- `messagediff`パッケージは異なる出力形式を使用するので、それを使用する場合（関数に`(want, got)`を渡した場合）はメッセージ`diff (want -> got)`が適切です。矢印の方向は「変更された」行の矢印の方向と一致するからです。

差分は複数行にまたがるので、差分を表示する前に改行を入れておくとよいでしょう。

### テストエラーセマンティクス

ユニットテストで文字列比較を行ったり`cmp`を使ったりして、特定の入力に対して特定の種類のエラーが返されることをチェックすると、将来そのエラーメッセージが書き換えられたときにテストがもろくなる可能性があります。これはユニットテストを変更検知器にしてしまう可能性があるので（[TotT: 変更検知器テストは有害と考える](https://testing.googleblog.com/2015/01/testing-on-toilet-change-detector-tests.html)参照）、関数が返すエラーの種類をチェックするために文字列比較を使わないようにしましょう。しかし、テスト対象のパッケージから送られてくるエラーメッセージが特定のプロパティを満たしているかどうかをチェックするために文字列比較を使用することは許されています（たとえば、パラメータ名が含まれているかどうかなど）。

Goのエラー値には通常、人間の目を意識したものと、セマンティクス的な制御フローを意識したものがあります。テストでは、人間のデバッグを目的とした情報を表示するのではなく、 確実に観測できるセマンティクス的な情報だけをテストするようにしましょう。セマンティクス的に意味のあるエラーを作成するためのガイダンスは、 [エラーに関するベストプラクティス](#TBD)を参照してください。セマンティクス的な情報が不十分なエラーがあなたのコントロール外の依存関係から発生している場合、 エラーメッセージの解析に頼るのではなく、 APIを改善するためにその依存関係の所有者に対してバグを報告することを検討してください。

ユニットテストでは、エラーが発生したかどうかだけを気にするのが一般的です。その場合は、エラーが発生することを想定したときにそのエラーが非nilであるかどうかをテストするだけで十分です。エラーがセマンティクス的に他のエラーと一致するかどうかをテストしたい場合は、 [`cmpopts.EquateErrors`](https://pkg.go.dev/github.com/google/go-cmp/cmp/cmpopts#EquateErrors)と一緒に`cmp`を使用することを検討してください。

注意: テストで[`cmpopts.EquateErrors`](https://pkg.go.dev/github.com/google/go-cmp/cmp/cmpopts#EquateErrors)を使用していても、`wantErr`の値がすべて`nil`あるいは`cmpopts.AnyError`である場合は、`cmp`の使用は[不要な機構](guide.md#最小限のメカニズム)です。期待されるフィールドを`bool`にすることでコードを簡素化します。そうすれば、`!=`を使用した単純な比較が可能になります。
...
[GoTip #13: チェックのためのエラーを設計する](https://google.github.io/styleguide/go/index.html#gotip)も参照ください。
...
多くの異なるテストケースを同じようなテストロジックでテストできる場合は、テーブル駆動テストを使用します。

- ある関数の実際の出力が期待される出力と等しいかどうかをテストする場合。たとえば、[`fmt.Sprintf`の多くのテスト](https://cs.opensource.google/go/go/+/master:src/fmt/fmt_test.go)や、以下のような最小限のスニペットがこれにあたります。
- ある関数の出力が常に同じ不変量集合に適合しているかどうかをテストする場合。たとえば、[`net.Dial.Sprintf`のテスト](https://cs.opensource.google/go/go/+/master:src/net/dial_test.go;l=318;drc=5b606a9d2b7649532fe25794fa6b99bd24e7697c)がそうです。

ここに、標準の文字列ライブラリからコピーしたテーブル駆動テストの最小構成があります。必要に応じて、別の名前を使ったりテストスライスをテスト関数に移動したり、 サブテストや設定・後始末関数のような追加機能を追加したりすることができます。常に、[有用なテストの失敗](#有用なテストの失敗)を念頭に置いてください。
...
**注意**: 上記のこの例の失敗メッセージは、[関数を識別し](#関数の識別)、[入力を識別する](#入力の識別)というガイダンスを満たしています。[行を数値で識別する](#TBD)必要はありません。
...
テストテーブルの中のテストのインデックスを、テストの名前や入力内容を表示するための代用品として使用しないでください。どのテストケースが失敗しているのかを知るために、 テストテーブルを調べて項目を数えるのは誰もやりたがらないでしょう。
...
- テストを foo_test.go ファイルに格納します。
- テストファイルには foo パッケージを使用します
- テスト対象のパッケージを明示的にインポートしません
...
同じパッケージのテストは、パッケージ内のエクスポートされていない識別子にアクセスすることができます。これにより、テストのカバレッジが向上し、より簡潔なテストが可能になるかもしれません。テストの中で宣言された[例](#例)は、ユーザーがコードで必要とするパッケージ名を持っていないことに注意しましょう。

#### 別パッケージのテスト

テスト対象となるコードと同じパッケージでテストを定義することは必ずしも適切ではありませんし、可能でさえありません。そのような場合は、パッケージ名に`_test`というサフィックスをつけます。これは、[パッケージ名](#パッケージ名)には「アンダースコアを使わない」というルールの例外です。たとえば、

- 統合テストが所属するライブラリが明らかでない場合
...
- 同じパッケージでテストを定義すると依存関係が循環してしまう場合
...
import (
  "fireworks"
  "fireworkstestutil" // fireworkstestutilもfireworksをインポートする
)
```

### `testing`パッケージの使用

Go標準ライブラリは、[`testing`パッケージ](https://pkg.go.dev/testing)を提供します。私のコードベースでは、Goのコードに許可されている唯一のテストフレームワークです。特に、[アサーションライブラリ](#アサーションライブラリ)やサードパーティのテストフレームワークは許可されていません。
...
これらは、[複合リテラル](https://go.dev/ref/spec#Composite_literals)や[初期化を伴うif](https://go.dev/ref/spec#If_statements)構文といった言語の中核機能と連携して、テスト作成者が「明確で、読みやすく、保守しやすい」テストを書くことができるように設計されています。

## 比決定事項

スタイルガイドでは、すべての事柄について明確な規定を列挙することはできませんし、意見を述べない事柄をすべて列挙することもできません。とはいえ、可読性のコミュニティが以前から議論し、コンセンサスを得られていない事柄をいくつか挙げてみましょう。

- **ローカル変数のゼロ値での初期化**。`var i int`` i := 0`は同等です。[初期化のベストプラクティス](#TBD)も参照してください。
- **空の複合リテラルと`new` または `make`**。`&File{}`と`new(File)` は等価です。`map[string]bool{}`と`make(map[string]bool)`も同様です。[複合宣言のベストプラクティス](#TBD)も参照してください。
- **`cmp.Diff`呼び出しの引数の順序**。ローカルに一貫性を持たせ、失敗メッセージに[凡例を含め](#差分をプリント)てください。
- **`errors.New`と書式なし文字列の`fmt.Errorf`**。`errors.New("foo")`と `fmt.Errorf("foo")`は同じ意味で使用することができます。

もし特殊な状況下でこのような問題が再び発生した場合、可読性メンターが任意のコメントをすることもありますが、一般的には記述者は与えられた状況下で好きなスタイルを自由に選択することができます。

もちろん、スタイルガイドでカバーされていないことでさらに議論が必要なことがあれば、記述者は特定のレビューで、あるいは内部の掲示板で質問することができます。
